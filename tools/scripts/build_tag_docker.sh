...
# bazel build //projects/devops_fastapi_app:tarball || exit 1
bazel build //projects/${1}:tarball || exit 1

# TAR_FILE=$(bazel cquery --output=files //projects/devops_fastapi_app:tarball)
# docker load --input ${TAR_FILE}
# TAR_PATH="$(bazel info bazel-bin)/projects/devops_fastapi_app/tarball"
TAR_PATH="$(bazel info bazel-bin)/projects/${1}/tarball"
