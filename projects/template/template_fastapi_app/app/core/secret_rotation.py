...
class SecretRotationManager:
    ...
    def __init__(
        ...
        key_lifetime_days: Optional[int] = None,
        enabled: bool = True,
    ):
        """
        ...
        self.transition_period = timedelta(
            days=transition_period_days if transition_period_days is not None 
            else settings.SECRET_KEY_TRANSITION_DAYS
        )
        self.key_lifetime = timedelta(
            days=key_lifetime_days if key_lifetime_days is not None 
        ...
        if secret_file_path is None:
            self.secrets_dir = os.environ.get(
                "SECRETS_DIR", 
                os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "secrets")
            )
        ...
        if not self.secrets[JWT_KEY_TYPE] and hasattr(settings, 'SECRET_KEY') and settings.SECRET_KEY:
    ...
    def _load_secrets(self) -> Dict[str, Any]:
        ...
        try:
            if os.path.exists(self.secret_file_path):
                ...
                logger.info(f"No secrets file found at {self.secret_file_path}, initializing new secrets")
    ...
    def _add_new_jwt_key(self) -> None:
        ...
        self.secrets[JWT_KEY_TYPE].insert(0, {
            "key": new_key,
            "created_at": now,
            "expires_at": (datetime.utcnow() + self.key_lifetime).isoformat(),
            "source": "rotation"
    ...
    def _add_new_db_credentials(self) -> None:
        ...
        self.secrets[DB_CREDENTIAL_TYPE].insert(0, {
    ...
    def _rotate_jwt_keys_if_needed(self) -> None:
        ...
        for i in sorted(keys_to_remove, reverse=True):
            logger.info(f"Removing expired JWT key created at {self.secrets[JWT_KEY_TYPE][i]['created_at']}")
    ...
    def _rotate_db_credentials_if_needed(self) -> None:
        ...
        for i in sorted(credentials_to_remove, reverse=True):
            logger.info(f"Removing expired database credentials created at {self.secrets[DB_CREDENTIAL_TYPE][i]['created_at']}")
