...
class FastAPIUser(HttpUser):
    ...
    wait_time = between(
        1, 3
    )  # Wait between 1 and 3 seconds between tasks (reduced for CI)
    access_token = None
    user_id = None
    item_ids = []
            ...
            for i in range(2):  # Reduced from 5 to 2
                with self.client.post(
                    ...
                    json={
                        ...
                        "description": f"Description for load test item {i}",
                    },
    ...
    def get_items(self):
        ...
        try:
            with self.client.get(
                "/api/v1/items/", name="/api/v1/items/ - Get Items", catch_response=True
            ) as response:
                if response.status_code != 200:
    ...
    def update_item(self):
        ...
        try:
            ...
            with self.client.put(
                ...
                if response.status_code != 200:
                    response.failure(
                        f"Failed to update item {item_id}: {response.text}"
                    )
        except Exception as e:
    ...
    def get_health_check(self):
        ...
        try:
            with self.client.get(
                "/health", name="/health - Health Check", catch_response=True
            ) as response:
                if response.status_code != 200:
...
if __name__ == "__main__":
    if not LOCUST_AVAILABLE:
        ...
        exit(1)
    print(
        "Run this load test with: locust -f tests/locustfile.py --host=http://localhost:8000"
    )
