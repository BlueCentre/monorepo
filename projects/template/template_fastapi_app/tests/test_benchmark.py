...
skip_reason = "Benchmarks skipped in CI environment. Set SKIP_BENCHMARKS=false to enable."
...
def test_item_crud_performance(db: Session, benchmark):
    ...
    try:
        ...
        def create_and_get_item():
            ...
            # Update the item
            crud_item.update(db=db, db_obj=stored_item, obj_in={"description": "Updated description"})
...
def test_api_list_performance(client: TestClient, db: Session, benchmark):
    ...
    try:
        ...
        token_response = client.post(
            "/api/v1/login/access-token",
            data={
                "username": "api_bench@example.com", 
                "password": "password123"
            },
            headers={"Content-Type": "application/x-www-form-urlencoded"},
        )
