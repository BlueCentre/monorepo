...
skip_contract = not PACTMAN_AVAILABLE or os.environ.get("SKIP_CONTRACT_TESTS", "false").lower() == "true"
    ...
    try:
        ...
        with pact:
            with mock.patch('pactman.verifier.path_exists', return_value=True):
                with mock.patch('pactman.verifier.request', side_effect=lambda **kw: mock_response(client, kw)):
                    ...
                    client.post(
                        "/api/v1/items/",
                        json={"title": "Contract Test Item", "description": "Item for contract testing"},
...
def test_login_contract(pact_setup, client: TestClient):
    ...
    try:
        ...
        pact.given(
            ...
            status=200,
            body={
                'access_token': pact.like('token_string'),
                'token_type': 'bearer'
            },
            headers={'Content-Type': 'application/json'}
        )
